			...
			<td>
			  <c:choose>
			    <c:when test="${itlike == 0}">
			      <input type="button" class = "btn btn_light" id="likebtn" value="스크랩 하기 " onclick="setScrap()">
				  <input type="hidden" id="likecheck" value="${itlike}">
			    </c:when>
			    
		        <c:when test="${itlike == 1}"> 
			      <input type="button" class = "btn btn_danger" id="likebtn" value="스크랩 취소" onclick="setScrap()">
			      <input type="hidden" id="likecheck" value="${itlike}">
			    </c:when>
			  </c:choose>
